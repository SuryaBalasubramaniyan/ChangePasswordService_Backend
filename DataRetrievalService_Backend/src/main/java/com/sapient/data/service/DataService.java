...
public class DataService {
	...
	public int changePassword(User user) {
		return dataDao.updatePassword(user.getPasswordHistory().getPassId(), user.getPasswordHistory().getPwd1(),
				user.getPasswordHistory().getSalt1(),user.getPasswordHistory().getPwd2(),
				user.getPasswordHistory().getSalt2(),user.getPasswordHistory().getPwd3(),
				user.getPasswordHistory().getSalt3());
	}
